...
As we have developed web applications at Cerner we have found that there are many questions that must be answered at the outset of an application's development and that many of those question have a common answer that can be provided by an application framework.
...
- How do we ensure modals are accessible and prevent multiple on the page at once? How do we can composable components flex between being presented as page content and modal content?
...
Applications commonly switch between light and dark modes. Terra provides theme APIs to allow themeing of custom components within a terra application.
