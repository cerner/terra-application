...
* [DefinePlugin](https://webpack.js.org/plugins/define-plugin/) - Plugin to define global compile-time values, including:
  ...
  * `TERRA_AGGREGATED_LOCALES` - The list of successfully aggregated locales available to the browser. See [i18n config](/dev_tools/terra-application-docs/webpack-config-terra/terra-i-18-n-config-js) for more details about the object structure.
  * `TERRA_THEME_CONFIG` - The scoped and default theme. See [theme config](/dev_tools/terra-application-docs/webpack-config-terra/terra-theme-config-js) for more details about the object structure.
