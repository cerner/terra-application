...
The ModalManager exposes its children to modal presentation APIs through the [DisclosureManagerContext](/components/terra-application/reference/contexts/disclosure-manager-context).
...
ModalManager monitors its disclosed content for the presence of rendered [NavigationPrompts](/components/terra-application/reference/components/navigation-prompt) within its disclosed content. ModalManager will ensure that the user is prompted prior to dismissing the presented modal if any [NavigationPrompts](/components/terra-application/reference/components/navigation-prompt) are rendered at the time of the dismissal request.
