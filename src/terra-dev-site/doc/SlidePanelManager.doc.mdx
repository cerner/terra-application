...
The SlidePanelManager exposes its children to panel presentation APIs through the [DisclosureManagerContext]().
...
SlidePanelManager renders an [ApplicationLoadingOverlayProvider]() around its disclosed components. Any [ApplicationLoadingOverlay]() components rendered by children will result in loading overlays being rendered by ApplicationBase.
...
SlidePanelManager monitors its disclosed content for the presence of rendered [NavigationPrompts]() within its disclosed content. SlidePanelManager will ensure that the user is prompted prior to dismissing the presented panel if any [NavigationPrompts]() are rendered at the time of the dismissal request.
