...
To add themes, create new files for each theme and include them in the original SCSS file. Theme files define variables that are applied when the theme becomes active. Themes must be locally scoped and must use a supported theme class name.
