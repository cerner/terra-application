...
`ApplicationEventEmitter` can be used to create and emit custom events to facilitate communications between the emitting component (emitter) and the listening component (listener). The behavior of the event emitter is similar to the "pub/sub" model. When ApplicationEventEmitter emits an event, listeners that have registered to listen to this event can act upon receiving the event.
...
|`off`|`eventEmitter.off('event-name', listener)`|Removes the specified listener from the listener array for the event named `event-name`. If there are multiple listens to this event, only one is removed.|
|`removeListener`|`eventEmitter.removeListener('event-name', listener)`|Similar to `off` except this removes all listeners for event named `event-name`.|
...
eventEmitter.on('event-name', listener);
eventEmitter.on('event-name', listener);
eventEmitter.off('event-name', listener);   // Removes the first listener of the event.
eventEmitter.emit('event-name');            // listenCount == 1
eventEmitter.off('event-name', listener);   // Removes the second listener of the event.
eventEmitter.emit('event-name');            // Ignored: listenCount == 1
```
