...
The SlidePanelManager exposes its children to panel presentation APIs through the [DisclosureManagerContext](/reference/terra-application/contexts/disclosure-manager-context).
...
SlidePanelManager renders an [ApplicationLoadingOverlayProvider](/reference/terra-application/components/application-loading-overlay) around its disclosed components. Any [ApplicationLoadingOverlay](/reference/terra-application/components/application-loading-overlay) components rendered by children will result in loading overlays being rendered by ApplicationBase.
...
SlidePanelManager monitors its disclosed content for the presence of rendered [NavigationPrompts](/reference/terra-application/components/navigation-prompt) within its disclosed content. SlidePanelManager will ensure that the user is prompted prior to dismissing the presented panel if any [NavigationPrompts](/reference/terra-application/components/navigation-prompt) are rendered at the time of the dismissal request.
