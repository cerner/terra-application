...
> if the width value is larger than the defined minimum, even if the width value is included in a higher breakpoint's range.
...
`terra-application` also provides a set of Sass mixins that define media queries for the supported breakpoints.

```scss
@import '~terra-breakpoints/lib/breakpoints/media-queries';
