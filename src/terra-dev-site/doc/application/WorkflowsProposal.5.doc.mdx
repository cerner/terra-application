...
1. Application Modal (Modal Manager)
...
(Root)   (Application Modal)     (Notification Dialog)     (Blocking Overlay)    (Framework Dialog)
...
2. Relative
  ...
  * Components include: Application Modal, Popup
...
The Application Modal component allows a component to present modal content within the same component context.
The Application Modal is considered an extension of the content that renders it, not a wholly separate entity
managed by the framework. The removal of the presenting component will result in the dismissal of the presented ApplicationModal.
...
* ApplicationModals presented within a ApplicationModal will result in stacked modal presentation
* Subsequent ApplicationModals presented at the same index will be automatically closed
...
* How do we get global modal banners (like the Demographics banner) injected into this content-driven modal?
  * We could have a separate context that components can provide with a banner, any ApplicationModals open within that context present the content.
...
The Popup component's intended usage is very similar to that of the ApplicationModal. The Popup should be used
...
const useBlockingOverlay = () => {
  const api = useMemo(() => ({
    show: (promise, options) => {
      ...
      TerraApplication.blockingOverlay.show(options, wrappedPromise.cancel);

      return {
        promise: wrappedPromise.promise.then((val) => {
          TerraApplication.blockingOverlay.hide();
          return val;
        }).reject((error) => {
          TerraApplication.blockingOverlay.hide();
          return error;
