...
The `userActionConfig` prop allows consumers have first class support for a action utility button.
If the `userActionConfig` prop is not set a action utility button will not be displayed.
