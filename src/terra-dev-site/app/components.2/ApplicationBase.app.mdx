...
ApplicationBase renders a theme context provider around its children. Children can access the current theme by using the [ThemeContext](/application/cerner-terra-application/contexts/theme-context).
...
ApplicationBase renders an [ActiveBreakpointContext.Provider](/application/cerner-terra-application/contexts/active-breakpoint-context) around its children. Children can access the current active breakpoint value by using the [ActiveBreakpointContext](/application/cerner-terra-application/contexts/active-breakpoint-context).

### Exception Handling

ApplicationBase renders an [ApplicationErrorBoundary](/application/cerner-terra-application/components/application-error-boundary) around its children. If the children throw exceptions during React lifecycle functions, ApplicationBase will catch the exceptions and render a styled error component in place of the children.

### Loading Overlays

ApplicationBase renders an [ApplicationLoadingOverlayProvider](/application/cerner-terra-application/components/application-loading-overlay-provider) around its children. Any [ApplicationLoadingOverlay](/application/cerner-terra-application/components/application-loading-overlay) components rendered by children will result in loading overlays being rendered by ApplicationBase.

### Status Views

ApplicationBase renders an [ApplicationStatusOverlayProvider](/application/cerner-terra-application/components/application-status-overlay-provider) around its children. Any [ApplicationStatusOverlay](/application/cerner-terra-application/components/application-status-overlay) components rendered by children will result in a status view being rendered by ApplicationBase.

### Unsaved Changes

ApplicationBase monitors its children for the presence of rendered [UnsavedChangesPrompts](/application/cerner-terra-application/components/unsaved-changes-prompt). ApplicationBase will prompt the user, if possible, using a browser-native confirmation dialog prior to any window unload event if any [UnsavedChangesPrompts](/application/cerner-terra-application/components/unsaved-changes-prompt) are rendered at the time of the unload request.
...
ApplicationBase renders a [ModalManager](/application/cerner-terra-application/components/modal-manager) around its children. Children can access modal disclosure APIs by using the [DisclosureManagerContext](/application/cerner-terra-application/contexts/disclosure-manager-context).