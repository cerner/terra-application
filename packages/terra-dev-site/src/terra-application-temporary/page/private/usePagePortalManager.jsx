...
const usePagePortalProvider = (rootContainerRef) => {
  ...
  const pagePortalContextValue = React.useMemo(() => {
    const renderPortalElement = (portalElement, portalId, ancestorPortalId, {
      ...
      portalRegisterRef.current[portalId] = {
        ...
        metaData,
        pagePortalLabelId,
        overflowData: portalRegisterRef.current[portalId] ? portalRegisterRef.current[portalId].overflowData : undefined,
      };
  ...
  React.useLayoutEffect(() => {
    ...
    // Iterate over the active portal identifiers, the first value indicating the bottom of the stack, the last index the first.
    ...
    for (let i = 0, count = danglingPortals.length; i < count; i += 1) {
      // If a previously registered portal is no longer present, we can safely delete our reference to it
      // after removing it from the container, if necessary.
    ...
    if (!activePortalArray.length) {
      // If no active portals remain, we remove the injected attribute as it likely points to an id that no longer exists.
      // This is likely infrequent beyond the initial mounting but we are covering our bases.
      rootContainerRef.current.removeAttribute('aria-labelledby');
    }

    lastActivePortalArrayRef.current = activePortalArray;
  }, [activePortalArray, rootContainerRef]);

  // We expose a dynamic component from the hook so that we do not need to expose the context or context value.
