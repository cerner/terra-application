...
The SlidePanelManager exposes its children to panel presentation APIs through the [DisclosureManagerContext](/components/terra-application/reference/contexts/disclosure-manager-context).
...
SlidePanelManager renders an [ApplicationLoadingOverlayProvider](/components/terra-application/reference/components/application-loading-overlay) around its disclosed components. Any [ApplicationLoadingOverlay](/components/terra-application/reference/components/application-loading-overlay) components rendered by children will result in loading overlays being rendered by ApplicationBase.
...
SlidePanelManager monitors its disclosed content for the presence of rendered [NavigationPrompts](/components/terra-application/reference/components/navigation-prompt) within its disclosed content. SlidePanelManager will ensure that the user is prompted prior to dismissing the presented panel if any [NavigationPrompts](/components/terra-application/reference/components/navigation-prompt) are rendered at the time of the dismissal request.
