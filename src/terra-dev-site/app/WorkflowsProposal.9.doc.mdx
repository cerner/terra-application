...
* UnsavedChangesPrompt integration with close actions
...
* (X) Rethink PageContainerActionsContext
  * It seems weird to have layouts implement Page-specific contexts, might make more sense the other way around
  * Layout-provided PageContainers might make this injection easier.
  * LayoutActionsContext
    * Object-based API, with key for custom classNames/data-attributes
    * still have separate start/end entries
    * allows PageContainer/Page to own action styles, and allows non-Pages to implement action as it sees fit to do so
...
* think about scroll persistance with unmounting navigation content
  * this might be impossible to ensure when content is unmounted, validate expectations with functional team
* Menu API is busted for checkmarks
  * Dave is fixing this, will need to consume new ActionMenu
* check nested PageContainer behavior
  * simplify PageContext API for Page/PageContainer interaction (onPageActivate?)
* Move over orion-application Features
  * About Box / OrionSessionProviderContext stuff
  * OrionRequestor mocking
* Splash Screen
* Page Layout APIs
  * Implicit vs Explicit Frames

  Page must have 0/1 Frames child
  Frame must have 0/n FrameSection children

  // 2 separate frames with Page overflow

    <StackedPageLayout>
    <FixedPageLayout>
    <GridPageLayout> // FUTURE

    <Page pageOverflowIsDisabled>
      <PageLayout>
        <CardLayout>
          <FrameSection label="Frame 1" actions={}>
            <Content />
          </FrameSection>
        </CardLayout>
      </PageLayout>
    ...
    <Page>
      <Frames type="stack">
        ...
        <Frame>
          <FrameSection label="Frame 2" actions={}>
            <Content />
          </FrameSection>
        </Frame
      </Frames>
    ...
    // 1 frame with Page overflow and subsections

    <Page>
      <Frames type="stack">
        <Frame>
          <FrameSection label="Section 1" actions={}>
            <Content />
          </FrameSection>
          <FrameSection label="Section 2" actions={}>
            <Content />
          </FrameSection>
          <FrameSection label="Section 3" actions={}>
            <Content />
          </FrameSection>
        </Frame>
      </Frames>
    ...
    // 1 frame with Frame overflow

    <Page>
      <Frames type="fixed">  // Page does prop inspection and infers presentation styles from type being fixed
        <Frame>
          <FrameSection label="Frame 1" actions={}>
            <Content />
          </FrameSection>
        </Frame>
        // Additional frames provided beyond the first will not be rendered, warning will be logged.
      </Frames>
      {showAnotherPage && <AnotherPage />}
      {showModal && <Modal />}
    </Page>

    // 4 frames with Page overflow and grid layout

    <Page>
      <Frames type="grid">
        <Frame stylesForGridLayout={...}>
          <FrameSection label="Section 1" actions={}>
            <Content />
          </FrameSection>
        </Frame>
        ...
        <Frame stylesForGridLayout={...}>
          <FrameSection label="Section 4" actions={}>
            <Content />
          </FrameSection>
        </Frame>
      </Frames>
    ...
    // 1 frame with frame-level tabs, 4 tab sections

    <Page>
      <Frames type="fixed">
        <TabsFrame>
          <Tab label="Tab 1" actions={}>
            <Content />
          </Tab>
          <Tab label="Tab 2" actions={}>
            <Content />
          </Tab>
          <Tab label="Tab 3" actions={}>
            <Content />
          </Tab>gtjtg
        </TabsFramtg djgjjgrjrgfguhfguudfv56g7ggt gte>
      </Frames>
      {showAnotherPage && <AnotherPage />}
      {showModal && <Modal />}
    </Page>

    // 1 frame with custom content
...
* Questions
  * Pages
    * Scrolling
      * How do we expect the scrollbar to render?
        **ANSWER** below header, in alignment with inset header divider
        * ^ this ain't gonna work, wait for input
      * Do we need the page header to be inset to match page body scrollbar?
        **ANSWER** no, if it's in alignment with the header divider
      * Do we need to account for extremely large header scenarios by setting a min-height on the page content?
        * resulting in vertical scroll in scroll in the worst case
        **ANSWER** yes, rough estimate is scroll on <40% available screen space

      Try putting overflow on PageLayout itself
        remove padding from width normalizer
        put min-width on cards themselves

    * Header Content
      * Do we want to enforce the rounded edges within the Page itself or is that a component concern?
        * **ANSWER** yes round toolbar, more specific toolbar APIs to come in the future
      * Do we anticipate more content going inside the header? label/actions/menu/toolbar/notification banners/?
        * **ANSWER** no, not reasonably anyway
      * notification banners
        * Are we cool with current design for initial release?
          * **ANSWER** No, pull in terra-alert, update to designs to come
        * Do we need to consider rollup patterns for alerts to mitigate min-height issue above?
          * **ANSWER** in the future, vertical overflow for now
        * If these notification banners are specifically for presentation at the Page-level, should we align API names to be Page-specific? (PageNotificationBanner?)
          * **ANSWER** No, NotificationBanner is agnostic, useNotificationBanners will need to change.
      * Do we care to scope out "Menu" presentation from the header for initial release until a better menu control can be created?
        * **ANSWER** scope out menu, leave roll-up, investigate dynamic page action roll-up

    * Page layouts
      * How important is the concept of the Card(?) to the overall Page design? Are there designs without Cards?
        * **ANSWER** likely yes to designs without cards
      * If Cards _are_ essential, how rigid to do we want/need to make the Page APIs? And how much do we want do now vs. later?
        * **ANSWER** layouts need to be mostly self-contained
      * Do we expect Cards to ever scroll internally? Do we expect content within a Card to ever remain fixed?
        * **ANSWER** cards will scroll internally, a layout will need to support 100% height layouts

    * Headerless Pages
      * Do these exist?
      * Do we create a separate component concept that does not include (or includes a subset of) Page API?
        * Notification banners are iffy here
      * **ANSWER** no headerless pages

    * Loading/Status Overlays
      * How can we allow Page implementers to easy present a loading indicator?
      * Do we need to change the loading indicator styles?
      * Do we need tighter integration with the Page to allow easier disabling of non-navigation actions?
      * Do status overlays/views make sense to be presented within a Page, enough so to have a first-class API for their presentation?
        * **ANSWER** YES
  * Modals
    * Touch base on Modal API
      * label/size/toolbar/onRequestClose are pretty solid
      * **ANSWER** no toolbar
    * Should modals be able to render Pages? Do the designs for the Page make sense within the Modal? Would we expect something different?
      * 
    * Same questions regarding loading/status overlays as with Pages
      * 

  * Workspace
    * Tabs Control
      * General tab design with active/pressed/focused states
      * Accessibility concerns with rollup and 2.1 support
    * Tab Content
      * How does workspace content scroll? Where are the scrollbars rendered?
        * **ANSWER** content scrolls under header, scrolling bound to workspace tab container
      * What sort of content restrictions/expectations do we need to know about to define the final API? Do we take anything or have a more rigid content API?
        * Decisions here will impact things like default padding/margin placement.
        * **ANSWER** no content restrictions, no "cards", punting thus far
      * Actions Menu for WorkspaceTab - Can we punt? If not, what sort of control are we needing? terra-menu has issues.
        * **ANSWER** we're punting

      
      * PageHeader uplift to use new ActionsMenu for rollup
      * Document portaled iframe behavior
      * add label ("Workspace"?) to workspace panel in secondary navigation layout
      * Include LegacyLayout (including loading/status overlays) for slow adopters
      * Make workspace expand to full container size at overlay sizes
      * Remove custom notification banner support in v2

      * Accessibility Talk
        * role='document' inside modal (?)
        * Modal bookmarks for tab-index wrapping from bottom->top, top->bottom

      * Think about adding ActiveMainPage handling to MainContainer
        * Making the "active main" concept not Page-specific, which I think makes more sense.
        * **DONE**
      * Revisit setState warning caused by Page's onBlur triggering during render 
        * maybe defer portaling until after render vs. during render 
        * **DONE** including major refactor for extensibility
...
* ApplicationBase
* ApplicationContainer
  * ApplicationConceptBannerContext
...
* SessionUserContext
* SessionActionsContext
* Suspense - with lifecycle callbacks
* TerraRequestor
* Transient Presentation - for external popup/content overlay dismissal
* Testing Helpers
...
Needs minor cleanup and testing. **3**

## ApplicationContainer

* Need to evaluate ApplicationConceptBannerContext API. **3**
* Need to evaluate ApplicationContainerContext API. **3**
* Need to evaluate Skip to Links for accessibility criteria. **5**
* Need to evaluate extensibility/plan for recreation in Mpages. **3**
* Need to investigate F6/region hopping accessibility feature and how those regions could be identified. ****8****
* Needs minor clean up and testing otherwise. ****5****

## Layouts

### PrimaryNavigationLayout

Rename? (ApplicationNavigationLayout)

* Need to evaluate ApplicationConceptBannerContext integration. **8**
* Need to evaluate SessionUserContext integration. **5**
* Need to evaluate SessionActionsContext integration. **5**
* Needs minor cleanup and testing. (Portal hook has been tested with workspace v1.) **3**

### SecondaryNavigationLayout

Rename? (SideNavigationLayout)

* Need to determine sidebar styles. UX
* Need to determine sidebar extensibility. UX
* Need to validate sidebar/workspace overlay behavior at small breakpoints, I know something is coming. UX
* Need to implement final workspace API. **5**
* Need to implement workspace container styles. **8**
* Need to better implement dynamic workspace resizing. **13**
* Needs major clean up and testing. **8**

### EmbeddedLayout

* Need to evaluate ApplicationConceptBannerContext integration. **3**
* Needs minor clean up and testing. **3**

### LegacyLayout

This is a layout that implements the ApplicationLayout-/ApplicationStatusOverlayProviders and an error boundary
for teams that want to migrate to v**2** without using pages.

* Needs to be developed in its entirety. **5**
* Needs testing. **3**

## Page

* Need to validate header "heading" level or determine if dynamic leveling is needed. UX
* Need to implement Page header styles. **5**
* Need to validate NotificationBanner integration. **3**
* Need to implement default PageLayout styles. **5**
* Need to implement loading/status overlays (matching WorkspaceContent API). **5**
* Need to validate provided contexts/callbacks for presentation state (when a Page is hidden due to subsequent drill-in) **8**
* Need to consume useElementSize hook for determining header action presentation state. **3**
* Needs cleanup and testing. **5**

## Layers

* Need to validate general layering strategy (with shared focus replay, inert logic, etc.) **12**

### Modal

* Need to determine modal header styles. UX
* Need to validate modal header "heading" level or determine if dynamic leveling is needed. UX
* Need to validate ported code (check for changes since copy). **3**
* Need to validate NotificationBanner integration. **3**
* Needs cleanup and testing. **5**
...
* Need to validate ported code (check for changes since copy). **3**
* Needs cleanup and testing. **3**

### InteractionBlockingOverlay

* Need to determine styles. UX
* Need to determine accessibility/flashing concerns. UX
* Need to validate positioning in overall layer hierarchy (above/below general notification dialog). UX
* Need to implement styles. **5**
* Need to determine presentation API (declarative vs. API). **8**
* Needs testing. **3**
...
* Tested with workspace v1, no changes anticipated at this time. **0**
...
Need to validate context shape and expected usage. **5**
...
Need to determine immediate need or punt.

* If needed, needs tests. **5**

## TerraRequestor

* Need to pull in fetch shim. **3**
* Need to determine API for global setup. **5**
* Need to code everything. **3**
* Need tests. **5**

## Transient Presentation - for external popup/content overlay dismissal

EventEmitter based API for dismissing transient content. Paired with custom useTransientState hook with listener built in. Need to determine need for this.

* If needed, need to determine API for this and tests. **5**

## Testing Helpers

It'd be nice to provide first-class mocks for testing with react-testing-library.
MockApplication with mock intl, etc.

* Need to spike on that. **8**

## Technical Documentation

* Need to stand up a doc site and write a whole bunch of stuff. **20**
